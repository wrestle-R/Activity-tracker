...
export default async function DashboardPage() {
  const { now, profile, workouts, runs, score } = await getAppData();
  const withinWeek = (date: string) => now - +new Date(date) < 7 * 86400000;
  const weeklyWorkouts = workouts.filter((item) => withinWeek(item.performed_at));
